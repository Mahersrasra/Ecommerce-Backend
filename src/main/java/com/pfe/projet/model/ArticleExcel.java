...
public class ArticleExcel {
    ...
    private void writeHeaderLine() {
        ...
        createCell(row, 2, "Catégorie", style);
        createCell(row, 3, "Prix Achat", style);  
        createCell(row, 4, "Prix Vente", style);  
        createCell(row, 5, "Tva", style);    
        createCell(row, 6, "Stock", style); 
        createCell(row, 7, "Sexe", style); 
        createCell(row, 8, "Taille", style); 
    ...
    private void writeDataLines() {
        ...
        for (ListArticle art  : listArticles) {
            ...
            createCell(row, columnCount++, art.getGenre(), style);
            createCell(row, columnCount++, art.getTaille(), style);
